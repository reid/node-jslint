...
function commandOptions() {
    var commandOpts = {
        ...
        jslintFlags = [
            ...
            'nomen', 'passfail', 'plusplus', 'regexp', 'rhino', 'sloppy',
            'stupid', 'sub', 'todo', 'unparam', 'vars', 'white'
        ],
...
function globFiles(list, callback) {
    async.map(list, function (element, callback) {
        ...
        result = result.filter(noNodeModules);
        callback(result);
    });
}
...
exports.runMain = function (options) {
    ...
    globFiles(options.argv.remain, function (files) {
        ...
        async.eachSeries(files, function (file, callback) {
            optModule.getOptions(process.env.HOME, path.dirname(file), options, function (procOptions) {
