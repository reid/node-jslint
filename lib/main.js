...
var FileOpener = require('./fileopener.js'),
...
// Set aliases to global objects so they can be mocked.
con = console;
pro = process;
...
function commandOptions() {
    var commandOpts = {
        ...
        nonStandardFlags = [
            'color', 'json', 'terse', 'version'
        ],
        booleanFlags = jslintFlags.concat(nonStandardFlags);

    booleanFlags.forEach(function (option) {
...
exports.runMain = function (options) {
    ...
    if (options.edition !== undefined && !linter.validateEdition(options.edition)) {
        process.nextTick(function () {
            con.warn('"' + options.edition + '" is not a valid JSLint edition.');
            con.warn('Valid editions are "' + linter.editions.reverse().join('", "') + '".');
            exit(1);
        });
        return;
    }
    ...
    globFiles(options.argv.remain, function (files) {
        ...
        // redundant IO.
        async.reduce(files, {}, function iterator(cache, file, callback) {
            var dirname = path.dirname(file);
            if (cache[dirname]) {
                callback(null, cache);
                return;
            }
            optModule.getOptions('/', dirname, options, function (procOptions) {
                cache[dirname] = procOptions;
                callback(null, cache);
            });
        }, function complete(error, cache) {
            /*jslint unparam: true */
            ...
            async.eachSeries(files, function iterator(file, callback) {
                var dirname = path.dirname(file),
                    ...
                    linter = new LintStream(cache[dirname]);
                opener.pipe(linter);
