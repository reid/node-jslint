...
function validateEdition(edition) {
    var valid = editions.indexOf(edition) === -1;
    if (valid) {
        con.warn('"' + edition + '" is not a valid JSLint edition.');
        con.warn('Valid editions are "' + editions.join('", "') + '".');
    }
    return valid;
}
exports.validateEdition = validateEdition;
